...
class HuggingfaceFinetuningTrainer():
    ...
    Attributes:
        ...
        _huggingface_config: Dictionary containing huggingface-specific configuration variables.
    """
    def __init__(self, general_config, huggingface_config):
        """Initializes the instance with configuration variables.
        Args:
            general_config:     Dictionary with general configuration variables.
            huggingface_config: Dictionary with huggingface-specific configuration variables.        """
    ...
    def run_finetuning(self):
        ...
        training_data   = self._load_data(self._general_config["training_input_file"])
        validation_data = self._load_data(self._general_config["validation_input_file"])
        test_data       = self._load_data(self._general_config["test_input_file"])
        ...
        training_args = transformers.TrainingArguments(
            output_dir                  = self._general_config["output_dir"],
            evaluation_strategy         = "epoch",
            per_device_train_batch_size = 4,
            per_device_eval_batch_size  = 32,
            num_train_epochs            = 1,
            logging_dir                 = self._huggingface_config["logging_dir"],
        ...
        trainer.save_model(self._huggingface_config["output_model"])  # Save core model
        tokenizer.save_pretrained(self._huggingface_config["output_model"])  # Save tokenizer
        lora_model.save_pretrained(self._huggingface_config["output_model"])  # Save LoRA model
        torch.save(lora_model.state_dict(), f"{self._general_config['output_dir']}/trained_model_state_dict.pth")
    ...
    def _load_tokenizer(self):
        tokenizer = transformers.AutoTokenizer.from_pretrained(
            pretrained_model_name_or_path = self._huggingface_config["checkpoint"],
            cache_dir                     = self._huggingface_config["cache_dir"],
            use_fast                      = False,
        )

        if self._huggingface_config["checkpoint"] == "meta-llama/Llama-2-7b-hf":
            tokenizer.add_special_tokens({"pad_token": tokenizer.eos_token})
        elif self._huggingface_config["checkpoint"] ==  "meta-llama/Meta-Llama-3-8B":
            tokenizer.add_special_tokens({"pad_token": tokenizer.eos_token})
    ...
    def _load_scaler(self):
        if self._general_config["evaluate_metric_rescaled"]:
            in_path = os.path.join(self._general_config["input_dir"], "scaler.pickle")
